...
func conditionalTasksPrompt(opts chatTaskOptions) string {
	...
	if opts.ActionPRChecksForDone {
		tasks = append(tasks, "- Action PR checks for done: after opening or updating the PR, you are not done. Use `gh` to inspect the PR's status checks and automated review activity, then wait for running checks to complete. If any check fails, fix it, commit, push, and wait again. If an automated AI review is running, wait for it to finish; fix every issue it reports above LOW severity, commit, push, and check again. Only finish when all checks pass and automated AI reviews contain no issues above LOW severity.")
	}
	if len(tasks) == 0 {
		return ""
	}
