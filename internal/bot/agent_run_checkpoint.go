...
// rebuild-and-resume instead of failing/deferring. Reconstruct only when the
// sandbox is genuinely gone (a permanent 404, not a transient outage —
// transient errors must keep deferring for retry) and the run is
// resume-eligible.
func (b *Bot) shouldReconstructLostSandbox(run runstore.Run, err error) bool {
	return isPermanentRecoverySandboxError(err) && b.resumeEligibleRun(run)
}

// handleRecoverySandboxGone is the recovery decision point when an interrupted
// run's sandbox cannot be fetched or restarted. When the sandbox is genuinely
// gone (a permanent 404, not a transient outage), it reconstructs a fresh
// sandbox and resumes; otherwise it preserves the existing behavior (fail on
// permanent errors, defer + retry on transient ones).
