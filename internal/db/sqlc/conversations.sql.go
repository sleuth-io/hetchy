...
const listActiveSandboxIDs = `-- name: ListActiveSandboxIDs :many
SELECT sandbox_id FROM conversations
WHERE sandbox_id != ''
  AND agent_state = 'running'
  AND agent_heartbeat_at > NOW() - $1::interval
`
...
func (q *Queries) ListActiveSandboxIDs(ctx context.Context, staleThreshold pgtype.Interval) ([]string, error) {
	rows, err := q.db.Query(ctx, listActiveSandboxIDs, staleThreshold)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
