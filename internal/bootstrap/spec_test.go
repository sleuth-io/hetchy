...
func TestNeedsGenerationUpgrade(t *testing.T) {
	...
	t.Run("generation above current does not need upgrade", func(t *testing.T) {
		// A spec written by a newer binary must not be force-regenerated by an
		// older one — the check is < not !=.
		spec := &Spec{BootstrapGeneration: CurrentBootstrapGeneration + 1}
		if NeedsGenerationUpgrade(spec) {
			t.Errorf("NeedsGenerationUpgrade(gen=%d) should return false when current=%d",
				spec.BootstrapGeneration, CurrentBootstrapGeneration)
		}
	})
}
